...
static void main_test_quadp(void);
...
static void main_test_quadp(void)
{
  ...
  /* Initialize function 'test_quadp' input arguments. */
  /* Initialize function input argument 'H'. */
  double H[9] = {1.0, -1.0, 1.0,
                 -1.0, 2.0, -2.0,
                  1.0, -2.0, 4.0};
  ...
  double ub[3] = {1.0, 1.0, 1.0};
  /* Call the entry-point 'test_quadp'. */

  test_quadp(H, f, lb, ub, x, &fval);
  printf("fval = %.4f, x = [%.4f %.4f %.4f]\n", fval, x[0], x[1], x[2]);
}
